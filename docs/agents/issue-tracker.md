...
`gh` infers the repo from the remotes and may prompt or guess when both are present. Pass `--repo JMAN730/opendroid` explicitly on any command where the target could be ambiguous.
